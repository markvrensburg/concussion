...
object Input {
  ...
  private def component(initialLength: Int) =
    ScalaComponent.builder[Props]("Input")
      .initialState(State(initialLength))
      .renderBackend[Backend]
      .build

  def apply(defaultValue: String = "",
            initialLength: Int = 20,
            onChange: String => Callback = _ => Callback(())
           ): Unmounted[Props, State, Backend] = {
    component(if (defaultValue.isEmpty) initialLength else defaultValue.length)(Props(defaultValue, onChange))
  }
