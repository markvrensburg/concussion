...
object Graph {
  ...
  protected[graph] final case class Overlay[V](left: Graph[V], right: Graph[V])
      extends Graph[V]
  protected[graph] final case class Connect[V](left: Graph[V], right: Graph[V])
      extends Graph[V]
  ...
  def overlay[V](g1: Graph[V], g2: Graph[V]): Graph[V] =
    (g1, g2) match {
  ...
  def connect[V](g1: Graph[V], g2: Graph[V]): Graph[V] =
    (g1, g2) match {
      case (Empty(), Empty())       => Empty()
      case (v @ Vertex(_), Empty()) => v
      case (Empty(), v @ Vertex(_)) => v
  ...
  def edges[A](es: Seq[(A, A)]): Graph[A] =
    es.foldLeft[Graph[A]](empty)(
      (g, e) => overlay(connect(vertex(e._1), vertex(e._2)), g)
    )
  ...
  def fold[A, B](empty: B,
                 ...
                 connect: (Graph[A], Graph[A]) => B)(graph: Graph[A]): B =
    graph match {
      case Empty() =>
        empty
      case Vertex(v) =>
  ...
  def foldg[A, B](empty: B,
    ...
    graph match {
      ...
      case Overlay(g1, g2) =>
        overlay(
          foldg(empty, vertex, overlay, connect)(g1),
          foldg(empty, vertex, overlay, connect)(g2)
        )
      case Connect(g1, g2) =>
        connect(
          foldg(empty, vertex, overlay, connect)(g1),
  ...
  def foldgEval[A, B](empty: B,
                      vertex: A => B,
                      overlay: (B, B) => B,
                      connect: (B, B) => B)(graph: Graph[A]): Eval[B] =
    graph match {
      case Empty() =>
        Eval.now(empty)
      case Vertex(v) =>
  ...
  def map[A, B](graph: Graph[A])(f: A => B): Graph[B] =
    foldg[A, Graph[B]](empty, f.andThen(vertex[B]), overlay[B], connect[B])(
      graph
    )

  def flatMap[A, B](graph: Graph[A])(f: A => Graph[B]): Graph[B] =
    foldg[A, Graph[B]](empty, f, overlay[B], connect[B])(graph)

  def toList[A](graph: Graph[A]): List[A] =
    foldg[A, List[A]](Nil, x => List(x), _ ++ _, _ ++ _)(graph)
  ...
  def circuit[A](vs: Seq[A]): Graph[A] =
    vs match {
      case Nil     => empty
  ...
  def vertexSet[A](graph: Graph[A]): Set[A] =
    foldg[A, Set[A]](Set.empty, Set(_), _ ++ _, _ ++ _)(graph)

  def edgeSet[A](graph: Graph[A]): Set[(A, A)] =
    graph match {
      case Empty()         => Set.empty
      case Vertex(_)       => Set.empty
      case Overlay(g1, g2) => edgeSet(g1) ++ edgeSet(g2)
      case Connect(g1, g2) =>
        edgeSet(g1) ++ edgeSet(g2) ++ (for {
          v1 <- vertexSet(g1)
          v2 <- vertexSet(g2)
        } yield (v1, v2))
    }
  ...
  def induce[A](f: A => Boolean)(graph: Graph[A]): Graph[A] =
    foldg[A, Graph[A]](
      empty,
  ...
  implicit val GraphIsMonad: Monad[Graph] = new Monad[Graph] {
    ...
    private def flatMapSafe[A, B](
      fa: Graph[A]
    )(f: A => Graph[B]): Eval[Graph[B]] =
      foldgEval[A, Graph[B]](empty, f, overlay[B], connect[B])(fa)

    override def tailRecM[A, B](a: A)(f: A => Graph[Either[A, B]]): Graph[B] =
      flatMapSafe(f(a)) {
        case Right(b)    => pure(b)
