...
sealed trait Graph[V, E] {
  ...
  def +(right: Graph[V, E])(implicit ev: Monoid[E]): Graph[V, E] =
    overlay(this, right)

  def *(right: (E, Graph[V, E])): Graph[V, E] =
  ...
  def induce(f: V => Boolean): Graph[V, E] =
    foldg[Graph[V, E]](empty, v => if (f(v)) vertex(v) else empty, (edge, g1, g2) => {
      (g1, g2) match {
        case (_, Empty()) => g1
        case (Empty(), _) => g2
  ...
  def removeVertex(v: V)(implicit ev: Eq[V]): Graph[V, E] =
    induce(_ =!= v)

  def focus(f: V => Boolean)(implicit ev1: Eq[E], ev2: Monoid[E]): Focus[V, E] =
    foldg[Focus[V, E]](emptyFocus, vertexFocus(f), connectFoci(_, _, _))

  def context(p: V => Boolean)(implicit ev1: Eq[E], ev2: Monoid[E]): Option[Context[V, E]] = {
    val f = focus(p)
    if (f.ok)
  ...
  def filterContext(
      ...
      is: V => Boolean,
      os: V => Boolean
  )(implicit ev1: Eq[V], ev2: Eq[E], ev3: Monoid[E]): Graph[V, E] =
    context(_ === v)
      .map(c => {
        overlays[V, E](
          List(
            vertex(v),
            induce(_ =!= v),
            edges[V, E](c.inputs.filter(x => is(x._2)).map(x => (x._1, x._2, v))),
            edges[V, E](c.outputs.filter(x => os(x._2)).map(x => (x._1, v, x._2)))
          )
        )
      })
      .getOrElse(this)
...
object Graph {
  ...
  protected[graph] final case class Empty[V, E]() extends Graph[V, E]
  protected[graph] final case class Vertex[V, E](vertex: V) extends Graph[V, E]
  ...
  def vertex[V, E](vertex: V): Graph[V, E] =
    Vertex[V, E](vertex)

  def edge[V, E](edge: E, vtx1: V, vtx2: V): Graph[V, E] =
  ...
  def vertices[V, E: Monoid](vs: Seq[V]): Graph[V, E] =
    overlays(vs.map(vertex[V, E]))

  def edges[V, E: Monoid](es: Seq[(E, V, V)]): Graph[V, E] =
    overlays(es.map(x => edge(x._1, x._2, x._3)))

  def overlays[V, E: Monoid](gs: Seq[Graph[V, E]]): Graph[V, E] =
    gs.foldLeft[Graph[V, E]](empty)(overlay)

  protected[graph] final case class Focus[V, E](
      ok: Boolean,
      is: List[(E, V)],
      os: List[(E, V)],
      vs: List[V]
  ...
  protected[graph] final case class Context[V, E](inputs: List[(E, V)], outputs: List[(E, V)])

  implicit def GraphEq[V: Eq, E: Eq: Monoid]: Eq[Graph[V, E]] =
    (x: Graph[V, E], y: Graph[V, E]) => x.toAdjacencyMap === y.toAdjacencyMap
