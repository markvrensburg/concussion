...
object Graph {
  ...
  //Generalized fold
  def foldg[A, B](empty: B, vertex: A => B, overlay: (B, B) => B, connect: (B, B) => B)(
      graph: Graph[A]
  ): B =
    graph match {
      case Empty() =>
  ...
  def foldgEval[A, B](empty: B, vertex: A => B, overlay: (B, B) => B, connect: (B, B) => B)(
      graph: Graph[A]
  ): Eval[B] =
    graph match {
      case Empty() =>
  ...
  def map[A, B](graph: Graph[A])(f: A => B): Graph[B] =
    foldg[A, Graph[B]](empty, f.andThen(vertex[B]), overlay[B], connect[B])(graph)
  ...
  def transpose[A](graph: Graph[A]): Graph[A] =
    foldg[A, Graph[A]](empty, vertex, overlay, (g1, g2) => connect(g2, g1))(graph)

  def induce[A](f: A => Boolean)(graph: Graph[A]): Graph[A] =
    foldg[A, Graph[A]](empty, v => if (f(v)) vertex(v) else empty, overlay, connect)(graph)
  ...
  implicit val GraphIsMonad: Monad[Graph] = new Monad[Graph] {
    ...
    private def flatMapSafe[A, B](fa: Graph[A])(f: A => Graph[B]): Eval[Graph[B]] =
