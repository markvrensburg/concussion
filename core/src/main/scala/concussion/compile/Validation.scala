...
object Validation {
  ...
  def extractExecutables[A](statements: Vector[Statement[A]]): Map[Int, Opcode[A]] =
    statements.zipWithIndex.mapFilter(s => s._1.opcode.map((s._2 + 1) -> _)).toMap

  def extractLabels[A](statements: Vector[Statement[A]]): Map[String, Int] =
    statements.zipWithIndex.mapFilter(s => s._1.label.map(_.name -> (s._2 + 1))).toMap
  ...
  def validateLabels[A](program: Program[A]): EitherNel[(Int, String), Program[A]] = {
